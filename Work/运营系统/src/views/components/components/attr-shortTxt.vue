<template>
  <div class="short-txt">
    <el-card shadow="hover">
      <el-form label-position="top" label-width="80px" :model="form" size="small" ref="form" :rules="rules">
        <el-form-item :label="dataInfo.atitle" prop="text">
          <el-input v-model="form.text" clearable placeholder="请输入内容" maxlength="20" show-word-limit></el-input>
        </el-form-item>
    ...
    var validateText = (rule, value, callback) => {
      if (value === "") {
        ...
        if (this.form.text.length > 20) {
          callback(new Error("请输入20字以内的内容"));
        } else {
          callback();
        }
      }
