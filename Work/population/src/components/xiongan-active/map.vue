...
export default {
  ...
  computed: {
    ...mapGetters([
      "xionganActiveMapCrumb",
      "xionganActiveTreeList",
      "xionganActivePopulationNumber"
    ])
  },
  watch: {
    dataList(val) {
    ...
    xionganActivePopulationNumber(val) {
      this.numberList = val;
      if (val) {
        this.setNumber(val);
      }
  ...
  methods: {
    setLevel(val) {
      ...
      var level2 = Math.ceil((max / 3) * 2.5);
      this.splitList = [
        {
          start: level2,
          end: max
    ...
    showProvinceMap() {
      this.showProvince()
      this.showMap("xiongAn.json");
      this.toCrumb(130100, 1, '雄安');
    },
    showCity() {
    ...
    showCitySearch(id, type, name) {
      this.areaName = name;
      this.xionganActiveTreeList.forEach(element => {
