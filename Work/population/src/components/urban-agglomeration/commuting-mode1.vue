...
export default {
  ...
  methods: {
    initData(val) {
      ...
      val.forEach((item, i) => {
        if (item.trvType != "步行" && item.trvType != "飞机") {
    ...
    changeCity() {
      [this.val.valueFrom, this.val.valueTo] = [this.val.valueTo, this.val.valueFrom]
        ...
        getData() {
        var that = this
        $.get('../../../static/map-json/bannerData2.json', function (Json) {
          that.fakeData = Json
          that.initData(that.fakeData[0].data)
          that.val.valueFrom = '雄安'
          that.val.valueTo = ['石家庄','保定'][0]
          that.echartsinit()
        });
        // for (let i = 0; i < 3; i++) {
        //   console.log(i)
        ...
        mySetInterval(function () {
          if (i == 2) {
           ...
           that.data = that.fakeData[i].data
          
            that.initData(that.data)
            that.val.valueFrom = '雄安'
            that.val.valueTo = ['石家庄','保定'][i]
            that.echartsinit()
          i++
        }, 3000)
      }
