<template>
  <div class="major-info">
    <!-- 设备信息 -->
    <el-form ref="majorForm" :model="majorInfo" :rules="rules" class="details-box" label-position="top" size="small">
      <el-row :gutter="20" class="row-item">
        <el-col :span="12">
          <el-form-item label="高压钳头链接模式" prop="HPTHLM">
            <el-select v-model="majorInfo.HPTHLM" filterable clearable style="width:100%" placeholder="请选择高压钳头链接模式"
        ...
        <el-col :span="12">
          <el-form-item :required="majorInfo.HPTHLM==2?true:false" label="抄表采集器设备编号" prop="ENOMRC">
            <el-select v-model="majorInfo.ENOMRC" clearable filterable style="width:100%" placeholder="请选择抄表采集器设备编号"
              @change="changeList">
              <el-option v-for="(item,index) in ENOMRC" :label="item.deviceNo" :value="item.deviceNo" :key="index" />
            </el-select>
          </el-form-item>
        </el-col>
      ...
      <el-row :gutter="20" class="row-item">
        <el-col :span="24">
          <el-form-item label="变压器表面温度测量装置编号" prop="SLOTSTMD">
            <el-select v-model="majorInfo.SLOTSTMD" multiple style="width:100%" placeholder="请选择变压器表面温度测量装置编号"
              @change="changeList3">
              <el-option v-for="(item,index) in SLOTSTMD" :label="item.deviceNo" :value="item.deviceNo" :key="index" />
            </el-select>
          </el-form-item>
        </el-col>
      </el-row>
      <el-row :gutter="20" class="row-item">
        <el-col :span="12">
          <el-form-item label="环境温度测量装置编号" prop="SNOATMD">
            <el-select v-model="majorInfo.SNOATMD" clearable filterable style="width:100%" placeholder="请选择环境温度测量装置编号"
              @change="changeList2">
              <el-option v-for="(item,index) in SNOATMD" :label="item.deviceNo" :value="item.deviceNo" :key="index" />
            </el-select>
          </el-form-item>
        </el-col>
        <el-col v-if="majorInfo.HPTHLM==1" :span="12">
          <el-form-item label="高压电流采集器编号" prop="NOHVBE">
            <el-select v-model="majorInfo.NOHVBE" filterable clearable style="width:100%" placeholder="请选择高压电流采集器编号"
              @change="changeList4">
              <el-option v-for="(item,index) in NOHVBE" :label="item.deviceNo" :value="item.deviceNo" :key="index" />
            </el-select>
          </el-form-item>
        </el-col>
      </el-row>
...
export default {
  ...
  watch: {
    ...
    "majorInfo.ENOMRC": function (n, o) {
      this.NOHVBE = [];
      this.NOHVBE = this.cbqList.filter((item) => item.deviceNo != n);
    },
    "majorInfo.NOHVBE": function (n, o) {
      this.ENOMRC = [];
      this.ENOMRC = this.cbqList.filter((item) => item.deviceNo != n);
    },
    "majorInfo.SLOTSTMD": {
      handler(n, o) {
        this.SNOATMD = [];
        this.SNOATMD = this.tempList.filter(
          (item) => !n.includes(item.deviceNo)
        );
      },
      deep: true,
    },
    "majorInfo.SNOATMD": function (n, o) {
      this.SLOTSTMD = [];
      this.SLOTSTMD = this.tempList.filter((item) => item.deviceNo != n);
    },
  },
  mounted() {
    this.initList();
